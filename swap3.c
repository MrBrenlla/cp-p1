...
void *swap(void *ptr)
{
	...
	pthread_mutex_lock(args->imutex);
	while((*args->iterations-=1)>=0) {
		pthread_mutex_unlock(args->imutex);
...
void start_threads(struct options opt)
{
	...
	for (i = 0; i < opt.num_threads; i++) {
		...
		args[i].iterations = &iteracions;
		args[i].imutex     = &imutex;
		args[i].mutex      = mutex;
